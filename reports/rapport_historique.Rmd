...
mapdeck_flux <- function(flux_df, width=350, height=350, ...) {
  ...
  mapdeck(min_zoom = 10, width=width, height=height, style = mapdeck_style("dark"), ... ) %>% 
...
paris <- group_map(flux_paris, ~mapdeck_flux(.x, token=NULL))
...
```{r}
mapdeck_flux(flux_lmnt %>% ungroup %>% filter(Annee=="2019"), width = 800, height = 800, zoom=5.2)
```
...
```{r}
mapdeck_flux(flux_region %>% ungroup %>% filter(Annee=="2019"), width = 800, height = 800, zoom=5.2)
```
